 ...
 * 
 * For example, writing a simple string trimming UDF might look like this:
 * 
 * <pre>
 *   {@code
 ...
 * </pre>
 * 
 * There is a lot of boilerplate to check the number of arguments and the
 * parameter types in the tuple.
 * 
 * Instead, with this class, you can derive from SimpleEvalFunc and create a
 * <code>call()</code> method (not exec!), just specifying the arguments as a
 * regular function. The class handles all the argument checking and exception
 * wrapping for you. So your code would be:
 * 
 * <pre>
 *   {@code
 * package datafu.pig.util;
...
public abstract class SimpleEvalFunc<T> extends EvalFunc<T> {
	...
	public SimpleEvalFunc() {
		...
		if (m == null)
			throw new IllegalArgumentException(String.format(
					"%s: couldn't find call() method in UDF.", getClass()
							.getName()));
	}

	// Pig can't get the return type via reflection (as getReturnType normally
	// tries to do), so give it a hand
	@Override
	...
	public T exec(Tuple input) throws IOException {
		...
		if (input.size() != pvec.length)
			throw new IOException(String.format(
					"%s: got %d arguments, expected %d.", _method_signature(),
					input.size(), pvec.length));
		...
		for (int i = 0; i < pvec.length; i++) {
			...
			try {
				...
				throw new IOException(
						String.format(
								"%s: argument type mismatch [#%d]; expected %s, got %s",
								_method_signature(), i + 1, pvec[i].getName(),
								o.getClass().getName()));
			}
			args[i] = o;
		}
		...
		try {
			...
			throw new IOException(String.format(
					"%s: caught exception processing input.",
					_method_signature()), e);
		}
	}

	/**
	 * Override outputSchema so we can verify the input schema at pig compile
	...
	public Schema outputSchema(Schema inputSchema) {
		if (inputSchema == null) {
			throw new IllegalArgumentException(String.format(
					"%s: null schema passed to %s", _method_signature(),
					getClass().getName()));
		}
		...
		if (inputSchema.size() != parameterTypes.length) {
			throw new IllegalArgumentException(String.format(
					"%s: got %d arguments, expected %d.", _method_signature(),
					inputSchema.size(), parameterTypes.length));
		}
